    ...
    for txtname in txtnames:
        with open(txtname, 'r') as lines:
            for line in lines:
                ...
                _img = os.path.join(datadir, 'slices', 'images', line.split(' ')[0].strip())
...
class DTDDataloader(data.Dataset):
    def __init__(self, path='DTD', transform=None, skip_mouse_id='-1', train=True, task_name='fdg_uptake_class'):
        ...
        if train:
            filename = [os.path.join(path, task_label_folder, 'all_mouse_data', 'train' + SPLIT + '.txt')] \
                if skip_mouse_id == "-1" else \
                [os.path.join(path, task_label_folder, f'without_mouse_{skip_mouse_id}', 'train' + SPLIT + '.txt')]
        else:
            filename = [os.path.join(path, task_label_folder, 'all_mouse_data', 'test' + SPLIT + '.txt')] \
                if skip_mouse_id == "-1" else \
                [os.path.join(path, task_label_folder, f'without_mouse_{skip_mouse_id}', 'test' + SPLIT + '.txt')]

        self.images, self.labels = make_dataset(filename, path, class_to_idx, task_name)
...
class Dataloder():
    def __init__(self, path, spatial_size, batchsize, skip_mouse_id, task_name):
        ...
        trainset_norandom = DTDDataloader(path, transform_train_norandom, skip_mouse_id, train=True, task_name=task_name)
        trainset = DTDDataloader(path, transform_train, skip_mouse_id, train=True, task_name=task_name)
        testset = DTDDataloader(path, transform_test, skip_mouse_id, train=False, task_name=task_name)
